...
class MatchUtil {
  ...
  /// <p>全球星：1349</p>
  /// <p>虚拟运营商：170</p>
  static const String REGEX_MOBILE_EXACT = "^((13[0-9])|(14[5,7])|(15[0-3,5-9])|(166)|(17[0,1,3,5-8])|(18[0-9])|(19[8-9]))\\d{8}";

  /// 正则：电话号码
  static const String REGEX_TEL = "^([0]{1}\\d{2,3}-)|([(]?[（]?([0]{1}\\d{2,3})?[)]?[）]?[-]?)\\d{7,8}";

  /// 正则：邮箱
  static const String REGEX_EMAIL = "^\\w+([-+.]\\w+)*@\\w+([-.]\\w+)*\\.\\w+([-.]\\w+)*";
...
//  private static final String LOGIN_PASSWORD = "^(?=.*\\d)(?=.*[a-zA-Z]).{6,16}$";
  static const String REGEX_PASSWORD = "^([A-Z]|[a-z]|[0-9]){6,16}";

  /// 正则：QQ号
  static const String REGEX_TENCENT_QQ = "[1-9][0-9]{4,10}";

  /// 正则：中国邮政编码
  static const String REGEX_ZIP_CODE = "[1-9]\\d{5}(?!\\d)";

  /// 正则：网址
  static const String REGEX_HTTP = "[a-zA-z]+://[^\s]*";

  /// 正则：中文真实姓名
  static final String REGEX_TURENAME = "^[\\u4e00-\\u9fa5]+";
  static final String REGEX_TURENAME_FEW = "^[\\u4e00-\\u9fa5]+[·•][\\u4e00-\\u9fa5]+"; //少数民族

  /// 正则：标签,长度必须是1-4位
  static const String REGEX_TAG = "^[\\w\\u4e00-\\u9fa5]{1,4}";
  ...
  static bool isMobile(String phone) {
    return isMatch(regex: REGEX_MOBILE_EXACT, input: phone);
  }

  /// 验证是否正确网址
  static bool isHttp(String http) {
    return isMatch(regex: REGEX_HTTP, input: http);
  }

  /// 验证是否正确电话号
  static bool isTel(String tel) {
    return isMatch(regex: REGEX_TEL, input: tel);
  }

  /// 验证是否正确邮箱
  static bool isEmail(String email) {
    return isMatch(regex: REGEX_EMAIL, input: email);
  }

  /// 验证是否正确密码
  static bool isPasswrodFormat(String password) {
    return isMatch(regex: REGEX_PASSWORD, input: password);
  }

  /// 验证是否正确QQ长度
  static bool isQQFormat(String qq) {
    return isMatch(regex: REGEX_TENCENT_QQ, input: qq);
  }

  /// 验证是否正确邮政编码
  static bool isZipCode(String zipCode) {
    return isMatch(regex: REGEX_ZIP_CODE, input: zipCode);
  }

  /// 验证是否正确姓名
  static bool isTureName(String name) {
    if (name.contains('·') || name.contains("•")) {
      return isMatch(regex: REGEX_TURENAME_FEW, input: name);
    }
    return isMatch(regex: REGEX_TURENAME, input: name);
  }

  /// 验证是否正确标签
  static bool isTag(String tag) {
    return isMatch(regex: REGEX_TAG, input: tag);
  }
  ...
  static bool _isNumeric(String num) {
    return isMatch(regex: "[0-9]*", input: num);
  }
